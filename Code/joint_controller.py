...
class JointController(Node):
    def __init__(self):
        ...
        self.point_cloud_sub = self.create_subscription(
            PointCloud2,
            '/scan/points',  # Point cloud topic
            self.point_cloud_callback,
            10
        )
        
        # Subscribe to red circle detection result
        ...
        # Subscribe to red circle detection information
        ...
        # Red circle detection state
        self.red_circle_detected = False
        self.red_circle_info = ""

        # Timer
        self.timer = self.create_timer(0.5, self.timer_callback)
        
        self.get_logger().info("Joint controller initialized - waiting for red circle detection messages")
    
    def detection_callback(self, msg):
        """Callback for red circle detection status"""
        detected = msg.data
        if detected and not self.red_circle_detected:
            self.get_logger().info("Red circle detected! Stopping robot movement.")
            # Stop all movement immediately when first detected
            self.set_thrust(forward=False, left=False, right=False)
        elif not detected and self.red_circle_detected:
            self.get_logger().info("Red circle no longer detected. Resuming movement.")
            
        self.red_circle_detected = detected
    
    def detection_info_callback(self, msg):
        """Callback for red circle detection information"""
        self.red_circle_info = msg.data
        self.get_logger().info(f"Red circle info: {self.red_circle_info}")

    def point_cloud_callback(self, msg):
        # If red circle detected, stop all movement
        if self.red_circle_detected:
            self.set_thrust(forward=False, left=False, right=False)
            return
    ...
    def timer_callback(self):
        # If red circle detected, stop all joint movement
        if self.red_circle_detected:
            position_joint2 = position_joint3 = position_joint4 = position_joint5 = 0.0
        else:
        ...
        self.get_logger().info(
            ...
            f"joint4: {position_joint4}, joint5: {position_joint5}, Red circle: {self.red_circle_detected}")
